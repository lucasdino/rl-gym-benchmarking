...
- ✅ Ability to save algorithms, videos, and training results following runs. Also integrated Weights & Biases (but built own visualizer that is better)
...
If you want to use Weights & Biases (W&B), create a `.env` file in this project root and set 'WANDB_API_KEY=your_key'. Make sure to update in `config.py` your W&B project name (will default to *RL-Gym-Benchmarking*).
...
- `algorithms/`: Core RL algorithms. Each RL algorithm (e.g., DDQN, SAC) has its own 'algorithm' file. You can see in `base.py` the structure of these files.
...
- `Action Sampler`: This expects inputs as 'Batch x Actions' and can sample using various methods like *greedy*, *epsilon-greedy*, or *Boltzmann (aka softmax w/ temperature)*.