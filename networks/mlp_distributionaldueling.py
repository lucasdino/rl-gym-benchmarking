...
class MLP_DistributionalDueling(nn.Module):
    def __init__(self, cfg: SingleNetworkConfig, obs_space, act_space):
        """ Combo of dueling nets (https://arxiv.org/pdf/1511.06581) and distributional rl (https://arxiv.org/pdf/1707.06887) """
        super(MLP_DistributionalDueling, self).__init__()
        self.cfg = cfg
    ...
    def forward(self, x):
        ...
        value = self.value_head(shared).view(-1, 1, self.num_atoms)
        action = self.action_head(shared).view(-1, self.act_space.n, self.num_atoms)
